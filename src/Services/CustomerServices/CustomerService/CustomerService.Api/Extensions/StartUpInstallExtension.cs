using Carter;
using CustomerService.Application.Assemblies;
using CustomerService.Application.Extension;
using CustomerService.Insfrastructure.Utilities.ApiDoc.Swagger;
using CustomerService.Insfrastructure.Utilities.Caching.Redis;
using CustomerService.Insfrastructure.Utilities.Cors;
using CustomerService.Insfrastructure.Utilities.HangFire;
using CustomerService.Insfrastructure.Utilities.Identity;
using CustomerService.Insfrastructure.Utilities.Logging;
using CustomerService.Insfrastructure.Utilities.MediatR;
using CustomerService.Insfrastructure.Utilities.ServiceBus;
using CustomerService.Insfrastructure.Utilities.Telemetry;
using CustomerService.Persistence.Context;
using CustomerService.Persistence.Extensions;
using CustomerService.Persistence.GenericRepository;
using CustomerService.Persistence.SearchEngine;
using CustomerService.Persistence.UnitOfWork;
using FluentValidation;

namespace CustomerService.Extensions
{
    public static class StartUpInstallExtension
    {
        public static async Task AddStartupServicesAsync(this WebApplicationBuilder builder)
        {
            builder.Services.AddCarter();
            builder.Services.AddControllers();
            builder.Services.AddHttpContextAccessor();
            builder.Services.AddEndpointsApiExplorer();
            builder.AddCustomSwaggerGen();
            builder.AddCors();
            builder.Services.AddMvc();
            //Log-Cache-Mediatr-FluentValidation-Mass transit
            builder.AddSerilog();
            builder.AddTelemeter();
            builder.AddRedis();
            var assembly = ApiAssemblyExtensions.GetLibrariesAssemblies();
            builder.AddHangFire(assembly);
            await builder.Services.ConfigureDbContextAsync(builder.Configuration);
            builder.AddMediatR(assembly);
            builder.Services.AddValidatorsFromAssembly(ApplicationAssemblyExtension.GetApplicationAssembly(), includeInternalTypes: true);
            builder.AddCustomMassTransit(assembly, (busRegistrationContext, busFactoryConfigurator) =>
            {
                busFactoryConfigurator.AddConsumers(busRegistrationContext);
                busFactoryConfigurator.AddPublishers();
            });
            builder.AddIdentitySettings();
            //Service Registered
            builder.Services.AddScoped<IUnitOfWork, UnitOfWork>();
            builder.Services.AddScoped<ICoreDbContext, CoreDbContext>();
            builder.Services.AddScoped(typeof(IRepository<>), typeof(Repository<>));
            await SearchEngineRegistration.MigrateElasticDbAsync(assembly, builder.Configuration);
            builder.Services.AddScoped<ICoreSearchEngineContext, CoreSearchEngineContext>();
        }
    }
}
